...
{
	public static Dictionary<Type, MethodInfo> OverrideDescription = [];
	public static void RegisterOverrideDescription(Type data) => OverrideDescription[data] = AccessTools.Method(typeof(ItemInfo), nameof(ItemInfo.Get)).MakeGenericMethod(data);
	...
	public string DataSize => Value.Length.SizeSuffix();
...
public class ItemInfo : IEnumerable<ItemData>
{
	...
	[UsedImplicitly] public static void GetTooltipPatch(ItemDrop.ItemData item, bool crafting, ref string __result)
	{
		 if (crafting || ItemDataManager.ItemData.OverrideDescription.Count == 0) return;
		 foreach (var kv in ItemDataManager.ItemData.OverrideDescription)
		 {
	...
	static ItemInfo()
	{
		...
		harmony.Patch(AccessTools.DeclaredMethod(typeof(ItemDrop.ItemData), nameof(ItemDrop.ItemData.GetTooltip), new[] { typeof(ItemDrop.ItemData), typeof(int), typeof(bool), typeof(float), typeof(int) }), postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(ItemInfo), nameof(GetTooltipPatch))));
		// Force loads
